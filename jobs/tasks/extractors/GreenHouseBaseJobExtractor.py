...
# from jobs.tasks.classes.job import Job
# from jobs.models import Job
from jobs.models import Job

class GreenHouseJobExtractor(BaseJobExtractor):
    ...
    def set_company_website(self, board):
        try:
            self.company_website = board.find(id='logo').find('a').get('href')
        except:
            self.company_website = ''

    def set_company_name(self, board):
        self.company_name = board.find('h1').text.strip()
    ...
    def extract_job_id(self, job):
        return self.base_url + job.find('a').get('href')


    def extract_job_ids(self, board):
        job_listings = board.find_all('div', class_='opening')
